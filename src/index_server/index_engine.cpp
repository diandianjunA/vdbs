...
void IndexEngine::init_gpu() {
    gpu_memory_manager = std::make_shared<GPUMemoryManager>(
        config->shard_num, config->size_data_per_element, config->size_per_shard);
    ...
    GlobalLogger->debug("size_data_per_element: {}", config->size_data_per_element);
...
IndexEngine::search_vectors(const std::vector<float> &query, int k,
    ...
    for (int i = 0; i < shard_num; i++) {
        threads.push_back(std::thread([&, i]() {
            auto [data, mutex] = gpu_memory_manager.get()->get_shard_buffer(i);
            ...
            int size_per_query = 100 * config->size_data_per_element;
            ...
            std::function<bool(std::vector<int>&)> check = [&](std::vector<int>& fetch_size) {
            ...
            while (check(fetch_size)) {
                ...
                for (int j = 0; j < query_num; j++) {
                    query_threads.push_back(std::thread([&, j]() {
                        ...
                        for (int z = 0; z < fetch_size[j]; z++) {
                            ...
                            pool.get()->rdma_read(
                                i, (char *)((char *)data + size_per_query * j),
                                size_per_query, entry * size_per_element);
                ...
                cuda_check((char *)data, entries[0], size_per_query / size_per_element);
                cuda_search((char *)data, query, query_num, ef_search, k,
                            size_per_query, visited_table, fetch_nodes,
                            fetch_size, entries, distances.data(), ids.data(),
                            found.data());
            }
        }));
    }
    ...
    // pool.get()->rdma_read(0, (char *)data, size, entry_node * size_per_element);
    // cuda_check((char *) data, entry_node, num);
    // return std::make_pair(std::vector<long>(), std::vector<float>());
}
